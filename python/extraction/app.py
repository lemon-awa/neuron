...
DEFAULT_CELL_TYPES_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "cell_types")

def parse_args():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description='Cell Type Navigation App')
    parser.add_argument('--cell-types-dir', 
                       type=str,
...
def main():
    ...
    with st.sidebar:
        ...
        pmid_files = [pmid for pmid in pmid_files if os.path.getsize(cell_types_dir / f"{pmid}.json") > 4]
            ...
            for cell_info in data:
                if cell_info["cell_type"] == selected_cell:
                    ...
                    for factoid in cell_info["factoids"]:
                        factoid_text = factoid.replace(selected_cell, f":blue[**{selected_cell}**]")
                        st.markdown(f"* {factoid_text}")
                    st.subheader("Evidence")
                    for evidence in cell_info["metadata"]["evidence"]:
                        evidence_text = get_evidence(pmid, evidence)
                        evidence_text = evidence_text.replace(selected_cell, f":blue[**{selected_cell}**]")
